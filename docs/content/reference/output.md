...
describes what users get. Pick a format with `-o`, or let dogapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dogapi <command> -o table     # a rounded, color-aware grid for reading
dogapi <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
dogapi <command> -o list      # one record per section, easy on the eyes
dogapi <command> -o jsonl     # one JSON object per line, for piping
dogapi <command> -o json      # a single JSON array
dogapi <command> -o csv       # spreadsheet friendly
dogapi <command> -o tsv       # tab-separated
dogapi <command> -o url       # just the URL column
dogapi <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
dogapi <command> --fields id,title,url
```
...
```bash
dogapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dogapi <command>            # a table, because this is a terminal
dogapi <command> | wc -l    # JSONL, because this is a pipe
```
